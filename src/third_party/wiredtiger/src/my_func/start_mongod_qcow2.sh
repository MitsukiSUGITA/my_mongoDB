...
# ★変更1: DB_PATHをローカル(qcow2内部)のパスに変更
...
echo "=================================================="
echo "      Starting Custom MongoDB on qcow2 Environment"
echo "=================================================="
...
# ★変更2: NFSのマウントチェックを削除（ローカルディレクトリの存在確認と作成に変更）
if [ ! -d "$DB_PATH" ]; then
    echo "ローカルデータディレクトリが存在しないため作成します: $DB_PATH"
    mkdir -p "$DB_PATH"
fi
